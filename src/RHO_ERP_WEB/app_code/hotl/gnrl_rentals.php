    ...
    if ($lgn_num > 0 && $canview === true) {
        if ($qstr == "DELETE") {
            ...
            if ($actyp == 1) {
                ...
                $hotlChckinDocSrvcTypID = isset($_POST['hotlChckinDocSrvcTypID']) ? (float) cleanInputData($_POST['hotlChckinDocSrvcTypID'])
                            : -1;
                ...
                $hotlChckinDocSpnsrSiteID = isset($_POST['hotlChckinDocSpnsrSiteID']) ? (float) cleanInputData($_POST['hotlChckinDocSpnsrSiteID'])
                            : -1;
                $hotlChckinDocSpnseeID = isset($_POST['hotlChckinDocSpnseeID']) ? (float) cleanInputData($_POST['hotlChckinDocSpnseeID']) : -1;
                $hotlChckinDocSpnseeSiteID = isset($_POST['hotlChckinDocSpnseeSiteID']) ? (float) cleanInputData($_POST['hotlChckinDocSpnseeSiteID'])
                            : -1;
                ...
                $scmSalesInvcEvntRgstrID = isset($_POST['scmSalesInvcEvntRgstrID']) ? (float) cleanInputData($_POST['scmSalesInvcEvntRgstrID'])
                            : -1;
                ...
                $scmSalesInvcDfltBalsAcntID = isset($_POST['scmSalesInvcDfltBalsAcntID']) ? (float) cleanInputData($_POST['scmSalesInvcDfltBalsAcntID'])
                            : -1;
                ...
                $hotlChckinDocPrntChcknID = isset($_POST['hotlChckinDocPrntChcknID']) ? (float) cleanInputData($_POST['hotlChckinDocPrntChcknID'])
                            : -1;
                $hotlChckinDocPrntChcknTyp = isset($_POST['hotlChckinDocPrntChcknTyp']) ? cleanInputData($_POST['hotlChckinDocPrntChcknTyp'])
                            : "";
                $scmSalesInvcRcvblDocID = isset($_POST['scmSalesInvcRcvblDocID']) ? (float) cleanInputData($_POST['scmSalesInvcRcvblDocID'])
                            : -1;
                ...
                //Create Main Item fo Header Check-In/Rent-Out
                if ($sbmtdHotlChckinDocID > 0 && $hotlChckinDocSrvcTypID > 0 && $sbmtdScmSalesInvcID > 0 && $hotlChckinDocRmID > 0) {
                    $oldCheckIn = isRoomsFree($hotlChckinDocRmID, $hotlChckinDocStrtDte, $hotlChckinDocEndDte);
                    if ($oldCheckIn > 0 && $oldCheckIn != $sbmtdHotlChckinDocID) {
                        $errMsg .= "For the Facility Number Selected, Reservation(s)/Rent Out(s)/Check-In(s)<br/> exist for the Date Period Specified hence cannot be Saved!";
                        //return false;
                    }
                    $mainItemID = -1;
                    if ($hotlChckinDocVchType == "Rent Out") {
                        $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "inv_item_id", $hotlChckinDocSrvcTypID);
                    } else if ($hotlChckinDocVchType == "Check-In") {
                        $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "inv_item_id", $hotlChckinDocSrvcTypID);
                    } else {
                        $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "no_shw_inv_itm_id",
                                        $hotlChckinDocSrvcTypID);
                    }
                    $mltplyAdlts = cnvrtBitStrToBool(getGnrlRecNm("hotl.service_types", "service_type_id", "mltply_dys_by_adults",
                                    $hotlChckinDocSrvcTypID));
                    $mltplyChldrn = cnvrtBitStrToBool(getGnrlRecNm("hotl.service_types", "service_type_id", "mltply_dys_by_chldrn",
                                    $hotlChckinDocSrvcTypID));
                    $noOfAdultsNumUpDwn = 0;
                    $noOfChdrnNumUpDwn = 0;
                    if ($hotlChckinDocStrtDte == $hotlChckinDocEndDte) {
                        $hotlChckinDocEndDte = str_replace("00:00:00", "23:59:59", $hotlChckinDocEndDte);
                    }
                    if ($sbmtdHotlChckinDocID > 0 && $mainItemID > 0) {
                        $rented_itm_qty = 1;
                        $rwItmRslt = get_ItemInf($mainItemID, $hotlChckinDocSpnsrSiteID);
                        while ($rwItm = loc_db_fetch_array($rwItmRslt)) {
                            $itmTyp = $rwItm[6];
                            $storeids = -1;
                            if ($itmTyp != "Services") {
                                $storeids = $selectedStoreID;
                            }
                            $ln_AstAcntID = (int) ($rwItm[10]);
                            $ln_CogsID = (int) ($rwItm[11]);
                            $ln_SalesRevID = (int) ($rwItm[12]);
                            $ln_SalesRetID = (int) ($rwItm[13]);
                            $ln_PurcRetID = (int) ($rwItm[14]);
                            $ln_ExpnsID = (int) ($rwItm[15]);
                            $itmNms = $rwItm[0];
                            $itmDescs = $rwItm[1];
                            $uomNm = $rwItm[9];
                            $sellingPrcs = (float) $rwItm[2];
                            $taxIDs = (int) ($rwItm[3]);
                            $taxNms = getGnrlRecNm("scm.scm_tax_codes", "code_id", "code_name", $taxIDs);
                            $dscntIDs = (int) ($rwItm[4]);
                            $dscntNms = getGnrlRecNm("scm.scm_tax_codes", "code_id", "code_name", $dscntIDs);
                            $chrgeIDs = (int) ($rwItm[5]);
                            $chrgeNms = getGnrlRecNm("scm.scm_tax_codes", "code_id", "code_name", $chrgeIDs);
                            $sysdte1 = $gnrlTrnsDteDMYHMS;
                            $dte1 = DateTime::createFromFormat('d-M-Y H:i:s', $hotlChckinDocStrtDte);
                            $dte2 = DateTime::createFromFormat('d-M-Y H:i:s', $hotlChckinDocEndDte);
                            $wrkngDte = null;
                            if ($hotlChckinDocCalcMthd == "Nights" || $hotlChckinDocCalcMthd == "0") {
                                $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y') . " 00:00:00")->modify('+1 day');
                            } else if ($hotlChckinDocCalcMthd == "Days" || $hotlChckinDocCalcMthd == "Months" || $hotlChckinDocCalcMthd == "Years"
                                    || $hotlChckinDocCalcMthd == "1" || $hotlChckinDocCalcMthd == "3" || $hotlChckinDocCalcMthd == "4") {
                                $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y') . " 00:00:00");
                            } else {
                                $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y H:i:s'));
                            }
                            if ($dte2 < $wrkngDte) {
                                $dte2 = DateTime::createFromFormat('d-M-Y H:i:s', $wrkngDte->format('d-M-Y H:i:s'))->modify('+1 hour');
                            }
                            $rwidx = -1;
                            $cmbine = true;
                            if ($cmbine && ($hotlChckinDocCalcMthd != "Hours" && $hotlChckinDocCalcMthd != "2")) {
                                $prevPrice = $sellingPrcs;
                                $extrDescs = $wrkngDte->format('d-M-Y H:i:s');
                                $cntr = 0;
                                $unitPrice = 0;
                                $tstPrice = 0;
                                $qty = 0;
                                //echo $wrkngDte->format('d-M-Y H:i:s') . "::" . $dte2->format('d-M-Y H:i:s');
                                while ($wrkngDte <= $dte2) {
                                    $tstPrice = getTrnsDatePrice($hotlChckinDocSrvcTypID, $wrkngDte->format('d-M-Y H:i:s'),
                                            $hotlChckinDocCalcMthd);
                                    if ($tstPrice < 0) {
                                    ...
                                    if (($tstPrice != $prevPrice) || $cntr == 0) {
                                        $prevPrice = $tstPrice;
                                        $unitPrice = round($scmSalesInvcExRate * $tstPrice, 2);
                                        if ($cntr == 0) {
                                            $extrDescs = $wrkngDte->format('d-M-Y H:i:s');
                                        } else {
                                            $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s', $wrkngDte->format('d-M-Y H:i:s'))->modify('-1 day')->format('d-M-Y H:i:s');
                                            $extrDescs .= $wrkngDte->format('d-M-Y H:i:s');
                                        }
                                        $idx = get_SalesDocLnID($mainItemID, $unitPrice, $sbmtdHotlChckinDocID);
                                        if ($idx <= 0) {
                                            $idx = getNewSalesInvcLnID();
                                            //echo "createSalesDocLn2:" . $qty;
                                            $afftctd4 += createSalesDocLn2($idx, $sbmtdScmSalesInvcID, $mainItemID, 1, $unitPrice,
                                                    $storeids, $scmSalesInvcInvcCurID, -1, $taxIDs, $dscntIDs, $chrgeIDs, "", ",", 0, true,
                                                    -1, $itmNms, $ln_AstAcntID, $ln_CogsID, $ln_SalesRevID, $ln_SalesRetID, $ln_PurcRetID,
                                                    $ln_ExpnsID, $rented_itm_qty, $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                        } else {
                                            $qty = 0;
                                            $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                            //echo "updateSalesDocLn2:" . $qty;
                                            $afftctd4 += updateSalesDocLn2($idx, $mainItemID, ($qty + 1), $unitPrice, $storeids,
                                                    $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                    $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                        }
                                        $cntr++;
                                    } else {
                                        $idx = get_SalesDocLnID($mainItemID, $unitPrice, $sbmtdHotlChckinDocID);
                                        if ($idx > 0) {
                                            $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty", $idx);
                                            $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                            //echo "updateSalesDocLn22:" . $qty;
                                            $afftctd4 += updateSalesDocLn2($idx, $mainItemID, ($qty + 1), $unitPrice, $storeids,
                                                    $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                    $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                        }
                                    ...
                                    $typIntrvl = "1 day";
                                    if ($hotlChckinDocCalcMthd == "Nights" || $hotlChckinDocCalcMthd == "Days" || $hotlChckinDocCalcMthd == "0"
                                            || $hotlChckinDocCalcMthd == "1") {
                                        $typIntrvl = "1 day";
                                    } else if ($hotlChckinDocCalcMthd == "Months" || $hotlChckinDocCalcMthd == "3") {
                                        $typIntrvl = "1 month";
                                    } else if ($hotlChckinDocCalcMthd == "Years" || $hotlChckinDocCalcMthd == "4") {
                                        $typIntrvl = "1 year";
                                    } else if ($hotlChckinDocCalcMthd == "Hours" || $hotlChckinDocCalcMthd == "2") {
                                        $typIntrvl = "1 hour";
                                    } else {
                                        $typIntrvl = "1 second";
                                        $wrkngDte = $dte2;
                                    }
                                    $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $wrkngDte->format('d-M-Y H:i:s'))->modify('+' . $typIntrvl);
                                    if ($wrkngDte >= $dte2) {
                                        $idx = get_SalesDocLnID($mainItemID, $unitPrice, $sbmtdHotlChckinDocID);
                                        $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty", $idx);
                                        $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s', $wrkngDte->format('d-M-Y H:i:s'))->modify('-' . $typIntrvl)->format('d-M-Y H:i:s');
                                        if ($mltplyAdlts && $noOfAdultsNumUpDwn > 0) {
                                            $qty = $qty * $noOfAdultsNumUpDwn;
                                        }
                                        if ($mltplyChldrn && $noOfChdrnNumUpDwn > 0) {
                                            $qty = $qty * $noOfChdrnNumUpDwn;
                                        }
                                        if ($idx > 0) {
                                            $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                            //echo "updateSalesDocLn222:" . $qty;
                                            $afftctd4 += updateSalesDocLn2($idx, $mainItemID, ($qty), $unitPrice, $storeids,
                                                    $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                    $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                        }
                                        break;
                            ...
                            } else if ($cmbine && $hotlChckinDocCalcMthd == "2") {
                                $prevPrice = $sellingPrcs;
                                $extrDescs = $wrkngDte->format('d-M-Y H:i:s');
                                $cntr = 0;
                                $unitPrice = 0;
                                ...
                                if (($tstPrice != $prevPrice) || $cntr == 0) {
                                    ...
                                    $idx = get_SalesDocLnID($mainItemID, $unitPrice, $sbmtdHotlChckinDocID);
                                    $qty = round(differenceInHours($dte1->format('d-M-Y H:i:s'), $dte2->format('d-M-Y H:i:s')), 4);
                                    if ($idx <= 0) {
                                        $idx = getNewSalesInvcLnID();
                                        //echo "createSalesDocLn21:" . $rented_itm_qty;
                                        $afftctd4 += createSalesDocLn2($idx, $sbmtdScmSalesInvcID, $mainItemID, $qty, $unitPrice, $storeids,
                                                $scmSalesInvcInvcCurID, -1, $taxIDs, $dscntIDs, $chrgeIDs, "", ",", 0, true, -1, $itmNms,
                                                $ln_AstAcntID, $ln_CogsID, $ln_SalesRevID, $ln_SalesRetID, $ln_PurcRetID, $ln_ExpnsID,
                                                $rented_itm_qty, $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                    } else {
                                        $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                        //echo "updateSalesDocLn21:" . $rented_itm_qty;
                                        $afftctd4 += updateSalesDocLn2($idx, $mainItemID, $qty, $unitPrice, $storeids,
                                                $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                    }
                                    ...
                                    if ($idx > 0) {
                                        $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                        //echo "updateSalesDocLn212:" . $rented_itm_qty;
                                        $afftctd4 += updateSalesDocLn2($idx, $mainItemID, $qty, $unitPrice, $storeids,
                                                $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                    }
                                ...
                                if ($wrkngDte >= $dte2) {
                                    $idx = get_SalesDocLnID($mainItemID, $unitPrice, $sbmtdHotlChckinDocID);
                                    $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty", $idx);
                                    $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s', $wrkngDte->format('d-M-Y H:i:s'))->modify('-1 second')->format('d-M-Y H:i:s');
                                    if ($mltplyAdlts && $noOfAdultsNumUpDwn > 0) {
                                        $qty = $qty * $noOfAdultsNumUpDwn;
                                    }
                                    if ($mltplyChldrn && $noOfChdrnNumUpDwn > 0) {
                                    ...
                                    if ($idx > 0) {
                                        $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "tax_code_id", $idx);
                                        //echo "updateSalesDocLn213:" . $rented_itm_qty;
                                        $afftctd4 += updateSalesDocLn2($idx, $mainItemID, ($qty), $unitPrice, $storeids,
                                                $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                $sbmtdHotlChckinDocID, $hotlChckinDocVchType, $extrDescs);
                                    }
                ...
                //var_dump($slctdFcltiesInfoLines);
                if (trim($slctdFcltiesInfoLines, "|~") != "" && $sbmtdHotlChckinDocID > 0) {
                    ...
                    for ($y = 0; $y < count($variousRows); $y++) {
                        ...
                        if (count($crntRow) == 12) {
                            $ln_TrnsLnID = (float) (cleanInputData1($crntRow[0]));
                            $ln_SrvcTypID = (float) cleanInputData1($crntRow[1]);
                            $ln_RoomID = (int) cleanInputData1($crntRow[2]);
                            $ln_RoomNum = cleanInputData1($crntRow[3]);
                            $ln_CstmrID = (int) cleanInputData1($crntRow[4]);
                            $ln_CstmrNm = cleanInputData1($crntRow[5]);
                            if (trim($ln_CstmrNm) == "") {
                                $ln_CstmrNm = "Unspecified";
                            }
                            $ln_StrtDte = cleanInputData1($crntRow[6]);
                            $ln_EndDte = cleanInputData1($crntRow[7]);
                            $hotlChckinDocPrntChcknID = $sbmtdHotlChckinDocID;
                            $hotlChckinDocPrntChcknTyp = $hotlChckinDocVchType;
                            if (trim($ln_StrtDte) == "") {
                            ...
                            $ln_DocNum = cleanInputData1($crntRow[9]);
                            $ln_CstmrSiteID = (int) cleanInputData1($crntRow[10]);
                            $rented_itm_qty = (float) cleanInputData1($crntRow[11]);
                            if ($rented_itm_qty == 0) {
                                $rented_itm_qty = 1;
                            }
                            if (trim($ln_DocNum) == "") {
                                $docTypes = array("Rent Out", "Check-In", "Reservation");
                                $docTypPrfxs = array("RT", "CI", "RS");
                                ...
                                $gnrtdTrnsNo = $gnrtdTrnsNo1 . str_pad(((getRecCount_LstNum("hotl.checkins_hdr", "doc_num", "check_in_id",
                                                        $gnrtdTrnsNo1 . "%") + 1) . ""), 3, '0', STR_PAD_LEFT);
                                $ln_DocNum = $gnrtdTrnsNo;
                            }
                            if ($ln_LineDesc == "") {
                                $ln_LineDesc = $hotlChckinDocVchType . " to Facility No. " . $ln_RoomNum . " for " . $ln_CstmrNm . " from " .
                                        $ln_StrtDte . " to " . $ln_EndDte . " (" . $ln_DocNum . ")";
                            }
                            if ($ln_LineDesc === "" || $ln_RoomID <= 0 || $ln_SrvcTypID <= 0) {
                                $errMsg .= "Row " . ($y + 1) . ":- Line Description and Facility Type and Number are all required Fields!<br/>";
                            }
                            /* if ($ln_CstmrID <= 0) {
                              $errMsg .= "Row " . ($y + 1) . ":- Person Issued To cannot be empty!<br/>";
                              } */
                            $oldCheckIn = isRoomsFree($ln_RoomID, $ln_StrtDte, $ln_EndDte);
                            if ($oldCheckIn > 0 && $oldCheckIn != $ln_TrnsLnID) {
                                $errMsg .= "Row " . ($y + 1) . ":-For the Facility Number Selected Reservation(s)/Rent Out(s)/Check-In(s)<br/> exist for the Date Period Specified hence cannot be Saved!";
                                //return false;
                            }
                            $mainItemID = -1;
                            if ($hotlChckinDocVchType == "Rent Out") {
                                $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "inv_item_id", $ln_SrvcTypID);
                            } else if ($hotlChckinDocVchType == "Check-In") {
                                $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "inv_item_id", $ln_SrvcTypID);
                            } else {
                                $mainItemID = (float) getGnrlRecNm("hotl.service_types", "service_type_id", "no_shw_inv_itm_id",
                            ...
                            if ($ln_StrtDte == $ln_EndDte) {
                                $ln_EndDte = str_replace("00:00:00", "23:59:59", $ln_EndDte);
                            }
                            //$sbmtdScmSalesInvcID;
                            if ($errMsg === "") {
                                //Create Checkin Doc
                                if ($ln_LineDesc != "" && $ln_RoomID > 0 && $ln_SrvcTypID > 0) {
                                    if ($ln_TrnsLnID <= 0) {
                                        $ln_TrnsLnID = createCheckIn($ln_DocNum, $hotlChckinDocVchType, $ln_StrtDte, $ln_EndDte,
                                                $ln_SrvcTypID, $ln_RoomID, $hotlChckinDocNoAdlts, $hotlChckinDocNoChldrn,
                                                $hotlChckinDocSpnsrID, $hotlChckinDocSpnsrSiteID, $ln_CstmrID, $ln_CstmrSiteID,
                                                $hotlChckinDocArvlFrm, $hotlChckinDocPrcdTo, $ln_LineDesc, $hotlChckinFcltyType,
                                                $hotlChckinDocStatus, $hotlChckinDocPrntChcknID, $hotlChckinDocPrntChcknTyp,
                                                $hotlChckinDocCalcMthd);
                                        $afftctd++;
                                    } else {
                                        $afftctd += updateCheckIn($ln_TrnsLnID, $ln_DocNum, $hotlChckinDocVchType, $ln_StrtDte, $ln_EndDte,
                                                $ln_SrvcTypID, $ln_RoomID, $hotlChckinDocNoAdlts, $hotlChckinDocNoChldrn,
                                                $hotlChckinDocSpnsrID, $hotlChckinDocSpnsrSiteID, $ln_CstmrID, $ln_CstmrSiteID,
                                                $hotlChckinDocArvlFrm, $hotlChckinDocPrcdTo, $ln_LineDesc, $hotlChckinFcltyType,
                                                $hotlChckinDocStatus, $hotlChckinDocPrntChcknID, $hotlChckinDocPrntChcknTyp,
                                ...
                                if ($ln_TrnsLnID > 0 && $mainItemID > 0) {
                                    ...
                                    while ($rwItm = loc_db_fetch_array($rwItmRslt)) {
                                        ...
                                        $chrgeNms = getGnrlRecNm("scm.scm_tax_codes", "code_id", "code_name", $chrgeIDs);
                                        $sysdte1 = $gnrlTrnsDteDMYHMS;
                                        $dte1 = DateTime::createFromFormat('d-M-Y H:i:s', $ln_StrtDte);
                                        $dte2 = DateTime::createFromFormat('d-M-Y H:i:s', $ln_EndDte);
                                        $wrkngDte = null;
                                        if ($hotlChckinDocCalcMthd == "Nights" || $hotlChckinDocCalcMthd == "0") {
                                            $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y') . " 00:00:00")->modify('+1 day');
                                        } else if ($hotlChckinDocCalcMthd == "Days" || $hotlChckinDocCalcMthd == "Months" || $hotlChckinDocCalcMthd
                                                == "Years" || $hotlChckinDocCalcMthd == "1" || $hotlChckinDocCalcMthd == "3" || $hotlChckinDocCalcMthd
                                                == "4") {
                                            $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y') . " 00:00:00");
                                        } else {
                                            $wrkngDte = DateTime::createFromFormat('d-M-Y H:i:s', $dte1->format('d-M-Y H:i:s'));
                                        ...
                                        if ($cmbine && ($hotlChckinDocCalcMthd != "Hours" && $hotlChckinDocCalcMthd != "2")) {
                                            ...
                                            //echo $wrkngDte->format('d-M-Y H:i:s') . "::" . $dte2->format('d-M-Y H:i:s');
                                            while ($wrkngDte <= $dte2) {
                                                $tstPrice = getTrnsDatePrice($ln_SrvcTypID, $wrkngDte->format('d-M-Y H:i:s'),
                                                        $hotlChckinDocCalcMthd);
                                                if ($tstPrice < 0) {
                                                    $tstPrice = $sellingPrcs;
                                                }
                                                if (($tstPrice != $prevPrice) || $cntr == 0) {
                                                    ...
                                                    if ($cntr == 0) {
                                                        ...
                                                        $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s',
                                                                        $wrkngDte->format('d-M-Y H:i:s'))->modify('-1 day')->format('d-M-Y H:i:s');
                                                        $extrDescs .= $wrkngDte->format('d-M-Y H:i:s');
                                                    }
                                                    $idx = get_SalesDocLnID($mainItemID, $unitPrice, $ln_TrnsLnID);
                                                    if ($idx <= 0) {
                                                        $idx = getNewSalesInvcLnID();
                                                        //echo "createSalesDocLn2:" . $qty;
                                                        $afftctd4 += createSalesDocLn2($idx, $sbmtdScmSalesInvcID, $mainItemID, 1,
                                                                $unitPrice, $storeids, $scmSalesInvcInvcCurID, -1, $taxIDs, $dscntIDs,
                                                                $chrgeIDs, "", ",", 0, true, -1, $itmNms, $ln_AstAcntID, $ln_CogsID,
                                                                $ln_SalesRevID, $ln_SalesRetID, $ln_PurcRetID, $ln_ExpnsID, $rented_itm_qty,
                                                                $ln_TrnsLnID, $hotlChckinDocVchType, $extrDescs);
                                                    } else {
                                                        $qty = 0;
                                                        $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id",
                                                                        "tax_code_id", $idx);
                                                        //echo "updateSalesDocLn2:" . $qty;
                                                        $afftctd4 += updateSalesDocLn2($idx, $mainItemID, ($qty + 1), $unitPrice, $storeids,
                                                                $scmSalesInvcInvcCurID, -1, $ln_TaxID, ",", true, $itmNms, $rented_itm_qty,
                                                    ...
                                                    $idx = get_SalesDocLnID($mainItemID, $unitPrice, $ln_TrnsLnID);
                                                    if ($idx > 0) {
                                                        $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty",
                                                                        $idx);
                                                        $ln_TaxID = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id",
                                                                        "tax_code_id", $idx);
                                                        //echo "updateSalesDocLn22:" . $qty;
                                                ...
                                                $typIntrvl = "1 day";
                                                if ($hotlChckinDocCalcMthd == "Nights" || $hotlChckinDocCalcMthd == "Days" || $hotlChckinDocCalcMthd
                                                        == "0" || $hotlChckinDocCalcMthd == "1") {
                                                    $typIntrvl = "1 day";
                                                } else if ($hotlChckinDocCalcMthd == "Months" || $hotlChckinDocCalcMthd == "3") {
                                                    $typIntrvl = "1 month";
                                                } else if ($hotlChckinDocCalcMthd == "Years" || $hotlChckinDocCalcMthd == "4") {
                                                ...
                                                if ($wrkngDte >= $dte2) {
                                                    ...
                                                    $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty", $idx);
                                                    $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s',
                                                                    $wrkngDte->format('d-M-Y H:i:s'))->modify('-' . $typIntrvl)->format('d-M-Y H:i:s');
                                                    if ($mltplyAdlts && $noOfAdultsNumUpDwn > 0) {
                                                        $qty = $qty * $noOfAdultsNumUpDwn;
                                                    }
                                                    if ($mltplyChldrn && $noOfChdrnNumUpDwn > 0) {
                                            ...
                                            if (($tstPrice != $prevPrice) || $cntr == 0) {
                                                ...
                                                $idx = get_SalesDocLnID($mainItemID, $unitPrice, $ln_TrnsLnID);
                                                $qty = round(differenceInHours($dte1->format('d-M-Y H:i:s'), $dte2->format('d-M-Y H:i:s')),
                                                        4);
                                                if ($idx <= 0) {
                                                    $idx = getNewSalesInvcLnID();
                                                    //echo "createSalesDocLn21:" . $rented_itm_qty;
                                                    $afftctd4 += createSalesDocLn2($idx, $sbmtdScmSalesInvcID, $mainItemID, $qty,
                                                            $unitPrice, $storeids, $scmSalesInvcInvcCurID, -1, $taxIDs, $dscntIDs,
                                                            $chrgeIDs, "", ",", 0, true, -1, $itmNms, $ln_AstAcntID, $ln_CogsID,
                                                            $ln_SalesRevID, $ln_SalesRetID, $ln_PurcRetID, $ln_ExpnsID, $rented_itm_qty,
                                                            $ln_TrnsLnID, $hotlChckinDocVchType, $extrDescs);
                                            ...
                                            if ($wrkngDte >= $dte2) {
                                                ...
                                                $qty = (float) getGnrlRecNm("scm.scm_sales_invc_det", "invc_det_ln_id", "doc_qty", $idx);
                                                $extrDescs .= "-" . DateTime::createFromFormat('d-M-Y H:i:s',
                                                                $wrkngDte->format('d-M-Y H:i:s'))->modify('-1 second')->format('d-M-Y H:i:s');
                                                if ($mltplyAdlts && $noOfAdultsNumUpDwn > 0) {
                                                    $qty = $qty * $noOfAdultsNumUpDwn;
                                                }
                                                if ($mltplyChldrn && $noOfChdrnNumUpDwn > 0) {
            ...
            if ($vwtyp == 0) {
                $srcModule = isset($_POST['srcModule']) ? cleanInputData($_POST['srcModule']) : 'allmodules';
                if ($srcModule === "allmodules") {
                    echo $cntent . "<li onclick=\"openATab('#allmodules', 'grp=$group&typ=$type&pg=$pgNo&vtyp=0');\">
                                    <span class=\"divider\"><i class=\"fa fa-angle-right\" aria-hidden=\"true\"></i></span>
                                    <span style=\"text-decoration:none;\">General Rentals</span>
				</li>
                               </ul>
                              </div>";
                }
                ...
                $extrWhere = " and a.doc_type IN ('Reservation','Rent Out', 'Check-In') and a.fclty_type IN ('Rental Item','Room/Hall','Field/Yard') 
      and a.prnt_chck_in_id<=0 and ((select count(1) from hotl.checkins_hdr xyz where xyz.prnt_chck_in_id = a.check_in_id)>0 or a.service_det_id <= 0)";
                //1000000;// 
                $total = get_Ttl_Checkins($srchFor, $srchIn, $orgID, $qShwUnpstdOnly, $qShwUnpaidOnly, $extrWhere, $qShwSelfOnly,
                        $qShwMyBranch);
                ...
                <form id='hotlChckinDocForm' action='' method='post' accept-charset='UTF-8'>
                    ...
                    <fieldset class=""><legend class="basic_person_lg1" style="color: #003245">ALL CHECK-IN/RENTAL DOCUMENTS</legend>
                        <div class="row" style="margin-bottom:0px;">
                            ...
                            <div class="<?php echo $colClassType3; ?>" style="padding:0px 15px 0px 15px !important;">
                                <div class="input-group">
                                    ...
                                    ?>" onkeyup="enterKeyFuncHotlChckinDoc(event, '', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');">
                                    <input id="hotlChckinDocPageNo" type = "hidden" value="<?php echo $pageNo; ?>">
                                    <input id="sbmtdScmRtrnSrcDocID" type = "hidden" value="-1">
                                    <label class="btn btn-primary btn-file input-group-addon" onclick="getHotlChckinDoc('clear', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');">
                                        <span class="glyphicon glyphicon-remove"></span>
                                    </label>
                                    <label class="btn btn-primary btn-file input-group-addon" onclick="getHotlChckinDoc('', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');">
                                    ...
                                    <select data-placeholder="Select..." class="form-control chosen-select" id="hotlChckinDocSrchIn">
                                        ...
                                        $valslctdArry = array("", "", "", "", "", "", "", "");
                                        $srchInsArrys = array(
                                            "Customer", "Facility Number", "Document Number",
                            ...
                            <div class="<?php echo $colClassType1; ?>">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination" style="margin: 0px !important;">
                                        <li>
                                            <a href="javascript:getHotlChckinDoc('previous', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" aria-label="Previous">
                                                <span aria-hidden="true">&laquo;</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="javascript:getHotlChckinDoc('next', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" aria-label="Next">
                        ...
                        <div class="row " style="margin-bottom:2px;padding:2px 15px 2px 15px !important">   
                            <div class="col-md-12" style="padding:2px 1px 2px 1px !important;border-top:1px solid #ddd;border-bottom:1px solid #ddd;">
                                <?php if ($canAdd === true && $srcModule === "allmodules") { ?>   
                                    <div class="col-md-5" style="padding:0px 0px 0px 0px !important;">                      
                                        <button type="button" class="btn btn-default" style="margin-bottom: 0px;" onclick="getOneHotlChckinDocForm(-1, 3, 'ShowDialog', 'Rent Out', 'RENT', '<?php echo $srcModule; ?>');" data-toggle="tooltip" data-placement="bottom" title="Add New Rent Out">
                                            <img src="cmn_images/add1-64.png" style="left: 0.5%; padding-right: 5px; height:20px; width:auto; position: relative; vertical-align: middle;">
                                            NEW RENT-OUT
                                        </button>                 
                                        <button type="button" class="btn btn-default" style="margin-bottom: 0px;" onclick="getOneHotlChckinDocForm(-1, 3, 'ShowDialog', 'Check-In', 'RENT', '<?php echo $srcModule; ?>');" data-toggle="tooltip" data-placement="bottom" title="Add New Check-In">
                                            <img src="cmn_images/add1-64.png" style="left: 0.5%; padding-right: 5px; height:20px; width:auto; position: relative; vertical-align: middle;">
                                            NEW CHECK-IN
                                        </button>                 
                                        <button type="button" class="btn btn-default" style="margin-bottom: 0px;" onclick="getOneHotlChckinDocForm(-1, 3, 'ShowDialog', 'Reservation', 'RENT', '<?php echo $srcModule; ?>');" data-toggle="tooltip" data-placement="bottom" title="Add New Reservation">
                                            <img src="cmn_images/add1-64.png" style="left: 0.5%; padding-right: 5px; height:20px; width:auto; position: relative; vertical-align: middle;">
                                            NEW RESERVATION
                                        </button> 
                                    </div>  
                                ...
                                <div class="col-md-2" style="padding:5px 1px 0px 1px !important;">
                                    <div class = "form-check" style = "font-size: 12px !important;">
                                        <label class = "form-check-label">
                                            ...
                                            ?>
                                            <input type="checkbox" class="form-check-input" onclick="getHotlChckinDoc('', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" id="hotlChckinDocShwUnpaidOnly" name="hotlChckinDocShwUnpaidOnly"  <?php echo $shwUnpaidOnlyChkd; ?>>
                                ...
                                <div class = "col-md-2" style = "padding:5px 1px 0px 1px !important;">
                                    <div class = "form-check" style = "font-size: 12px !important;">
                                        <label class = "form-check-label">
                                            ...
                                            ?>
                                            <input type="checkbox" class="form-check-input" onclick="getHotlChckinDoc('', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" id="hotlChckinDocShwUnpstdOnly" name="hotlChckinDocShwUnpstdOnly"  <?php echo $shwUnpstdOnlyChkd; ?>>
                                    ...
                                    <div class = "col-md-2" style = "padding:5px 1px 0px 1px !important;">
                                        <div class = "form-check" style = "font-size: 12px !important;">
                                            <label class = "form-check-label">
                                                ...
                                                ?>
                                                <input type="checkbox" class="form-check-input" onclick="getHotlChckinDoc('', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" id="hotlChckinDocShwSelfOnly" name="hotlChckinDocShwSelfOnly"  <?php echo $shwSelfOnlyChkd; ?>>
                                                Only Self-Created
                                            </label>
                                        </div>                            
                                    </div>
                                    ...
                                    <div class = "col-md-1" style = "padding:5px 1px 0px 1px !important;">
                                        <div class = "form-check" style = "font-size: 12px !important;">
                                            <label class = "form-check-label" title="Show Only My Branch">
                                                ...
                                                ?>
                                                <input type="checkbox" class="form-check-input" onclick="getHotlChckinDoc('', '#<?php echo $srcModule; ?>', 'grp=<?php echo $group; ?>&typ=<?php echo $type; ?>&pg=<?php echo $pgNo; ?>&vtyp=<?php echo $vwtyp; ?>&srcModule=<?php echo $srcModule; ?>');" id="hotlChckinDocShwMyBranchOnly" name="hotlChckinDocShwMyBranchOnly"  <?php echo $shwMyBranchOnlyChkd; ?>>
                                                My Branch
                                            </label>
                                        </div>                            
                                    </div>
                                            ...
                                            <tr id="hotlChckinDocHdrsRow_<?php echo $cntr; ?>">                                    
                                                ...
                                                <td class="lovtd">
                                                    <button type="button" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="bottom" title="View/Edit Document" 
                                                            onclick="getOneHotlChckinDocForm(<?php echo $row[0]; ?>, 3, 'ShowDialog', '<?php echo $row[14]; ?>', 'RENT', '<?php echo $srcModule; ?>');" style="padding:2px !important;" style="padding:2px !important;">                                                                
                                                                <?php
                                                                if ($canAdd === true) {
                                                                    ?>                                
                                                ...
                                                if ($row[15] == "Reserved" || $row[15] == "Checked-In" || $row[15] == "Ordered" || $row[15] == "Rented Out") {
                                                    $style2 = "color:green;";
                ...
                //New Rentals Form
                //var_dump($_POST);
                $sbmtdln_FcltyType = isset($_POST['ln_FcltyType']) ? cleanInputData($_POST['ln_FcltyType']) : '';
                $sbmtdln_SrvsTypID = isset($_POST['ln_SrvsTypID']) ? (float) cleanInputData($_POST['ln_SrvsTypID']) : -1;
                $sbmtdln_RoomID = isset($_POST['ln_RoomID']) ? (float) cleanInputData($_POST['ln_RoomID']) : -1;
                $srcModule = isset($_POST['srcModule']) ? cleanInputData($_POST['srcModule']) : 'allmodules';

                $sbmtdHotlChckinDocID = isset($_POST['sbmtdHotlChckinDocID']) ? (float) cleanInputData($_POST['sbmtdHotlChckinDocID']) : -1;
                $hotlChckinDocVchType = isset($_POST['hotlChckinDocVchType']) ? cleanInputData($_POST['hotlChckinDocVchType']) : "Rent Out";
                $hotlChckinFcltyType = "Rental Item";
                if (trim($sbmtdln_FcltyType) !== "") {
                    $hotlChckinFcltyType = trim($sbmtdln_FcltyType);
                }
                ...
                $hotlChckinDocEndDte = DateTime::createFromFormat('d-M-Y H:i:s', $gnrlTrnsDteDMYHMS)->modify('1 day')->format('d-M-Y H:i:s');
                ...
                $vPsblValID = getEnbldPssblValID("Default Rental Calc Method", getLovID("All Other Hospitality Setups"));
                $vPsblVal = getPssblValDesc($vPsblValID);
                if ($vPsblVal === "Nights") {
                    $vPsblVal = "0";
                } else if ($vPsblVal === "Days") {
                    $vPsblVal = "1";
                } else if ($vPsblVal === "Hours") {
                    $vPsblVal = "2";
                } else if ($vPsblVal === "Months") {
                    $vPsblVal = "3";
                } else if ($vPsblVal === "Years") {
                    $vPsblVal = "4";
                } else if ($vPsblVal === "Term") {
                    $vPsblVal = "5";
                }
                $vPsblValID1 = getEnbldPssblValID("Default Check-In Calc Method", getLovID("All Other Hospitality Setups"));
                $vPsblVal1 = getPssblValDesc($vPsblValID1);
                if ($vPsblVal1 === "Nights") {
                    $vPsblVal1 = "0";
                } else if ($vPsblVal1 === "Days") {
                    $vPsblVal1 = "1";
                } else if ($vPsblVal1 === "Hours") {
                    $vPsblVal1 = "2";
                } else if ($vPsblVal1 === "Months") {
                    $vPsblVal1 = "3";
                } else if ($vPsblVal1 === "Years") {
                    $vPsblVal1 = "4";
                } else if ($vPsblVal1 === "Term") {
                    $vPsblVal1 = "5";
                }
                if ($hotlChckinDocVchType == "Rent Out") {
                    $hotlChckinDocCalcMthd = $vPsblVal;
                    $hotlChckinFcltyType = "Rental Item";
                    $hotlChckinDocStatus = "Rented Out";
                } else if ($hotlChckinDocVchType == "Check-In") {
                    $hotlChckinDocCalcMthd = $vPsblVal1;
                    $hotlChckinFcltyType = "Room/Hall";
                    $hotlChckinDocStatus = "Checked-In";
                } else if ($hotlChckinDocVchType == "Reservation") {
                    $hotlChckinDocCalcMthd = $vPsblVal1;
                    $hotlChckinFcltyType = "Room/Hall";
                ...
                <form class="form-horizontal" id="oneScmSalesInvcEDTForm">
                    <fieldset class="basic_person_fs2" style="min-height:50px !important;">
                        <div class="row" style="margin-top:5px;">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-5" style="padding:0px 15px 0px 1px;">
                                        <select data-placeholder="Select..." class="form-control chosen-select" id="hotlChckinFcltyType">
                                            <?php
                                            $valslctdArry = array("", "", "");
                                            $srchInsArrys = array("Rental Item", "Room/Hall", "Field/Yard");
                                            for ($z = 0; $z < count($srchInsArrys); $z++) {
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="hotlChckinDocSpnsr" class="control-label col-md-4">Client/Sponsor:</label>
                                    <div  class="col-md-8">
                                        <div class="input-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        <div class="input-group"  style="width:100%;">
                                            <textarea class="form-control" rows="3" cols="20" id="hotlChckinDocOthrInfo" name="hotlChckinDocOthrInfo" <?php echo $mkRmrkReadOnly; ?> style="text-align:left !important;"><?php echo $hotlChckinDocOthrInfo; ?></textarea>
                            ...
                            <div class = "col-md-4">   
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <span  id="myScmSalesInvcStatusBtn" class="btn btn-default" style="white-space: normal !important;word-wrap:break-word !important; color:<?php echo $rqstatusColor; ?>;font-weight: bold;height:34px;width:100% !important;font-size:11px;">
                                            <?php
                                            echo $rqStatus . ($scmSalesInvcIsPstd == "1" ? " [Posted]" : " [Not Posted]") . " [" . $hotlChckinDocStatus . "]";
                                            ?>
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    <div class="tab-content" style="padding:3px 5px 2px 5px!important;">
                                        <div class="row">
                                            <div class="col-md-12">
                                                ...
                                                $nwRowHtml22 = "<tr id=\"oneSalesRqstdItmLinesRow__WWW123WWW\" onclick=\"$('#allOtherInputData99').val($('#oneSalesRqstdItmLinesLinesTable tr').index(this));\">                                    
                                                                        <td class=\"lovtd\"><span>New</span></td>                                              
                                                                        <td class=\"lovtd\"  style=\"\">  
                                                                            <input type=\"hidden\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_TrnsLnID\" value=\"-1\" style=\"width:100% !important;\">
                                                                            <input type=\"hidden\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTypID\" value=\"-1\" style=\"width:100% !important;\">  
                                                                            <input type=\"hidden\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_RoomID\" value=\"-1\" style=\"width:100% !important;\">    
                                                                            <input type=\"hidden\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_CstmrID\" value=\"-1\" style=\"width:100% !important;\">    
                                                                            <input type=\"hidden\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_CstmrSiteID\" value=\"-1\" style=\"width:100% !important;\"> 
                                                                            <div class=\"input-group date form_date_tme\" data-date=\"\" data-date-format=\"dd-M-yyyy hh:ii:ss\" data-link-field=\"dtp_input2\" data-link-format=\"yyyy-mm-dd hh:ii:ss\" style=\"width:100% !important;\">
                                                                                    <input class=\"form-control\" size=\"16\" type=\"text\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_StrtDte\" value=\"\">
                                                                                    <!--<span class=\"input-group-addon\"><span class=\"glyphicon glyphicon-remove\"></span></span>-->
                                                                                    <span class=\"input-group-addon\"><span class=\"glyphicon glyphicon-calendar\"></span></span>
                                                                                </div> 
                                                                        </td> 
                                                                        <td class=\"lovtd\" style=\"text-align: right;\">
                                                                                <div class=\"input-group date form_date_tme\" data-date=\"\" data-date-format=\"dd-M-yyyy hh:ii:ss\" data-link-field=\"dtp_input2\" data-link-format=\"yyyy-mm-dd hh:ii:ss\" style=\"width:100% !important;\">
                                                                                    <input class=\"form-control\" size=\"16\" type=\"text\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_EndDte\" value=\"\">
                                                                                    <!--<span class=\"input-group-addon\"><span class=\"glyphicon glyphicon-remove\"></span></span>-->
                                                                                    <span class=\"input-group-addon\"><span class=\"glyphicon glyphicon-calendar\"></span></span>
                                                                                </div> 
                                                                        </td>    
                                                                        <td class=\"lovtd\">
                                                                                <div class=\"input-group\" style=\"width:100% !important;\">
                                                                                    <input type=\"text\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTyp\" name=\"oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTyp\" value=\"\" readonly=\"true\" style=\"width:100% !important;\">
                                                                                    <label class=\"btn btn-primary btn-file input-group-addon\" onclick=\"getLovsPage('myLovModal', 'myLovModalTitle', 'myLovModalBody', 'Hospitality Service Types', 'allOtherInputOrgID', 'hotlChckinFcltyType', '', 'radio', true, '', 'oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTypID', 'oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTyp', 'clear', 1, '', function () {});\">
                                                                                        <span class=\"glyphicon glyphicon-th-list\"></span>
                                                                                    </label>
                                                                                </div>                                              
                                                                        </td>  
                                                                        <td class=\"lovtd\">
                                                                                <div class=\"input-group\" style=\"width:100% !important;\">
                                                                                    <input type=\"text\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_RoomNum\" name=\"oneSalesRqstdItmLinesRow_WWW123WWW_RoomNum\" value=\"\" readonly=\"true\" style=\"width:100% !important;\">
                                                                                    <label class=\"btn btn-primary btn-file input-group-addon\" onclick=\"getRoomNumLovsPage('oneSalesRqstdItmLinesRow_WWW123WWW_StrtDte', 'oneSalesRqstdItmLinesRow_WWW123WWW_EndDte', 'myLovModal', 'myLovModalTitle', 'myLovModalBody', 'Hospitality Facility Numbers', 'oneSalesRqstdItmLinesRow_WWW123WWW_SrvcTypID', '', '', 'radio', true, '', 'oneSalesRqstdItmLinesRow_WWW123WWW_RoomID', 'oneSalesRqstdItmLinesRow_WWW123WWW_RoomNum', 'clear', 1, '', function () {});\">
                                                                                        <span class=\"glyphicon glyphicon-th-list\"></span>
                                                                                    </label>
                                                                                </div>                                             
                                                                        </td>
                                                                        <td class=\"lovtd\" style=\"text-align: right;\">
                                                                            <input type=\"text\" class=\"form-control rqrdFld jbDetAccRate\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_RentedQty\" name=\"oneSalesRqstdItmLinesRow_WWW123WWW_RentedQty\" value=\"1\" onkeypress=\"gnrlFldKeyPress(event, 'oneSalesRqstdItmLinesRow_WWW123WWW_RentedQty', 'oneSalesRqstdItmLinesTable', 'jbDetAccRate');\" style=\"width:100% !important;text-align: right;\">                                                    
                                                                        </td>    
                                                                        <td class=\"lovtd\">
                                                                                <div class=\"input-group\" style=\"width:100% !important;\">
                                                                        ...
                                                                        <td class=\"lovtd\">
                                                                            <input type=\"text\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_LineDesc\" name=\"oneSalesRqstdItmLinesRow_WWW123WWW_LineDesc\" value=\"\" style=\"width:100% !important;text-align: left;\">                                                    
                                                                        </td> 
                                                                        <td class=\"lovtd\">
                                                                            <input type=\"text\" class=\"form-control\" aria-label=\"...\" id=\"oneSalesRqstdItmLinesRow_WWW123WWW_DocNum\" name=\"oneSalesRqstdItmLinesRow_WWW123WWW_DocNum\" value=\"\" style=\"width:100% !important;text-align: left;\" readonly=\"true\">                                                    
                                                                        </td>
                                                                        <td class=\"lovtd\">
                                                                        ...
                                                                        <td class=\"lovtd\" style=\"text-align: center;\">
                                                                            <button type=\"button\" class=\"btn btn-default\" style=\"margin: 0px !important;padding:0px 3px 2px 4px !important;\" onclick=\"delHotlChckinDocDetLn('oneSalesRqstdItmLinesRow__WWW123WWW');\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Delete Document Line\">
                                                                                <img src=\"cmn_images/no.png\" style=\"height:15px; width:auto; position: relative; vertical-align: middle;\">
                                                                            </button>
                                                                        </td>
                                                                    </tr>";
                                                $nwRowHtml22 = urlencode($nwRowHtml22);
                                                ...
                                                <div class="col-md-12" style="padding:0px 0px 0px 0px !important;">
                                                    <div class="col-md-4" style="padding:0px 0px 0px 0px !important;float:left;">
                                                        <?php if ($canEdt) { ?>
                                                            ...
                                                            <button id="addNwHotlChckinDocSmryBtn" type="button" class="btn btn-default" style="margin-bottom: 1px;height:30px;" onclick="insertNewHotlChckinDocRows('oneSalesRqstdItmLinesTable', 0, '<?php echo $nwRowHtml22; ?>');" data-toggle="tooltip" data-placement="bottom" title = "New Facility Booking">
                                                    ...
                                                    <div class="col-md-4" style="padding:0px 0px 0px 0px !important;">
                                                        <div class="" style="padding:0px 0px 0px 0px;float:right !important;"> 
                                                            ...
                                                            if ($rqStatus == "Not Validated") {
                                                                ?>
                                                                <?php if ($canEdt) { ?>
                                                                    <button type="button" class="btn btn-default" style="margin-bottom: 1px;height:30px;" onclick="saveHotlChckinDocForm('<?php echo $fnccurnm; ?>', 0, 'NORMAL', 'RENT', '<?php echo $srcModule; ?>');"><img src="cmn_images/FloppyDisk.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Save&nbsp;</button>    
                                                                <?php } ?>
                                                                <?php if ($canRvwApprvDocs) { ?>
                                                                    <button type="button" class="btn btn-default" style="margin-bottom: 1px;height:30px;" onclick="saveHotlChckinDocForm('<?php echo $fnccurnm; ?>', 2, 'NORMAL', 'RENT', '<?php echo $srcModule; ?>');" data-toggle="tooltip" data-placement="bottom" title="Finalize Document">
                                                                ...
                                                                if ($canPayDocs === true && $scmSalesInvcCstmrID > 0 && ($scmSalesInvcVchType
                                                                        == "Sales Invoice" || $scmSalesInvcVchType == "Sales Order" || $scmSalesInvcVchType
                                                                        == "Pro-Forma Invoice")) {
                                                                    ?>
                                                                    <button type="button" class="btn btn-default" style="margin-bottom: 1px;height:30px;" onclick="getOneAccbPayInvcForm(<?php echo $sbmtdScmRcvblsInvcID; ?>, 'Customer Payments', 'ShowDialog', -1, <?php echo $sbmtdHotlChckinDocID; ?>, 'Sales Invoice-Hospitality', 'XX_UNDEFINED_XX', '', 'hotlChckinDocSpnsrID', 'scmSalesInvcInvcCur', 'NO', 'SALES', 'RENT|<?php echo $srcModule; ?>||-1|-1');" data-toggle="tooltip" data-placement="bottom" title="Take Deposit">
                                                                ...
                                                                if ($canPayDocs === true && $scmSalesInvcOustndngAmnt > 0 && $scmSalesInvcVchType
                                                                        == "Sales Invoice") {
                                                                    ?>
                                                                    <button type="button" class="btn btn-default" style="margin-bottom: 1px;height:30px;" onclick="getOneAccbPayInvcForm(<?php echo $sbmtdScmRcvblsInvcID; ?>, 'Customer Payments', 'ShowDialog', -1, <?php echo $sbmtdHotlChckinDocID; ?>, 'Sales Invoice-Hospitality', 'XX_UNDEFINED_XX', '', 'hotlChckinDocSpnsrID', 'scmSalesInvcInvcCur', 'NO', 'SALES', 'RENT|<?php echo $srcModule; ?>||-1|-1');" data-toggle="tooltip" data-placement="bottom" title="Pay Document">
                                                    ...
                                                    <table class="table table-striped table-bordered table-responsive" id="oneSalesRqstdItmLinesTable" cellspacing="0" width="100%" style="width:100%;min-width: 400px !important;">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th style="">Facility Type</th>
                                                                <th style="">Facility Number</th>
                                                                <th style="max-width:70px;width:70px;text-align: right;">Rented Item QTY</th>
                                                                <th style="min-width:200px;">Person Issued to/ Person Responsible</th>
                                                                <th style="min-width:200px;">Destination/Location /Description/Remarks</th>
                                                                <th style="">Document Number</th>
                                                                <th style="">Status</th>
                                                                <th style="">Facility Return</th>
                                                        ...
                                                        <tbody>   
                                                            <?php
                                                            $cntr = 0;
                                                            $resultRw = get_One_CheckinChckns($sbmtdHotlChckinDocID);
                                                            ...
                                                            if ($ttlRows > 0) {
                                                                while ($rowRw = loc_db_fetch_array($resultRw)) {
                                                                    ...
                                                                    $trsctnLnStrtDte = $rowRw[4];
                                                                    $trsctnLnEndDte = $rowRw[5];
                                                                    $trsctnLnSrvcTyp = $rowRw[7];
                                                                    $trsctnLnSrvcTypID = (float) $rowRw[6];
                                                                    $trsctnLnFcltyNum = $rowRw[9];
                                                                    $trsctnLnFcltyID = (float) $rowRw[8];
                                                                    $trsctnLnCstmrRspnsbl = $rowRw[24];
                                                                    $trsctnLnCstmrID = (float) $rowRw[14];
                                                                    $trsctnLnCstmrSiteID = (float) $rowRw[15];
                                                                    $trsctnLnDesc = $rowRw[18];
                                                                    $trsctnLnDocNum = $rowRw[1];
                                                                    $trsctnLnDocStatus = $rowRw[20];
                                                                    $ln_RentedQty = (float) $rowRw[25];
                                                                    $cntr += 1;
                                                                    ?>
                                                                    <tr id="oneSalesRqstdItmLinesRow_<?php echo $cntr; ?>" onclick="$('#allOtherInputData99').val($('#oneSalesRqstdItmLinesLinesTable tr').index(this));">                                    
                                                                        ...
                                                                        <td class="lovtd"  style="">  
                                                                            ...
                                                                            <input type="hidden" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTypID" value="<?php echo $trsctnLnSrvcTypID; ?>" style="width:100% !important;">  
                                                                            <input type="hidden" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RoomID" value="<?php echo $trsctnLnFcltyID; ?>" style="width:100% !important;">    
                                                                            <input type="hidden" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_CstmrID" value="<?php echo $trsctnLnCstmrID; ?>" style="width:100% !important;">    
                                                                            <input type="hidden" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_CstmrSiteID" value="<?php echo $trsctnLnCstmrSiteID; ?>" style="width:100% !important;"> 
                                                                            <?php
                                                                            if ($canEdt === true) {
                                                                                ?>
                                                                                <div class="input-group date form_date_tme" data-date="" data-date-format="dd-M-yyyy hh:ii:ss" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd hh:ii:ss" style="width:100% !important;">
                                                                        ...
                                                                        <td class="lovtd">
                                                                            ...
                                                                            if ($canEdt === true) {
                                                                                ?>
                                                                                <div class="input-group" style="width:100% !important;">
                                                                                    <input type="text" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTyp" name="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTyp" value="<?php echo $trsctnLnSrvcTyp; ?>" readonly="true" style="width:100% !important;">
                                                                                    <label class="btn btn-primary btn-file input-group-addon" onclick="getLovsPage('myLovModal', 'myLovModalTitle', 'myLovModalBody', 'Hospitality Service Types', 'allOtherInputOrgID', 'hotlChckinFcltyType', '', 'radio', true, '', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTypID', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTyp', 'clear', 1, '', function () {});">
                                                                                        <span class="glyphicon glyphicon-th-list"></span>
                                                                                    </label>
                                                                                </div>      
                                                                            <?php } else { ?>
                                                                                <span><?php echo $trsctnLnSrvcTyp; ?></span>
                                                                            <?php } ?>                                             
                                                                        </td>  
                                                                        <td class="lovtd">
                                                                            <?php
                                                                            if ($canEdt === true) {
                                                                                ?>
                                                                                <div class="input-group" style="width:100% !important;">
                                                                                    <input type="text" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RoomNum" name="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RoomNum" value="<?php echo $trsctnLnFcltyNum; ?>" readonly="true" style="width:100% !important;">
                                                                                    <label class="btn btn-primary btn-file input-group-addon" onclick="getRoomNumLovsPage('oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_StrtDte', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_EndDte', 'myLovModal', 'myLovModalTitle', 'myLovModalBody', 'Hospitality Facility Numbers', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_SrvcTypID', '', '', 'radio', true, '', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RoomID', 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RoomNum', 'clear', 1, '', function () {});">
                                                                                        <span class="glyphicon glyphicon-th-list"></span>
                                                                                    </label>
                                                                                </div>      
                                                                            <?php } else { ?>
                                                                                <span><?php echo $trsctnLnFcltyNum; ?></span>
                                                                            <?php } ?>                                             
                                                                        </td>
                                                                        <td class="lovtd" style="text-align: right;">
                                                                            <input type="text" class="form-control rqrdFld jbDetAccRate" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RentedQty" name="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RentedQty" value="<?php echo $ln_RentedQty; ?>" onkeypress="gnrlFldKeyPress(event, 'oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_RentedQty', 'oneSalesRqstdItmLinesTable', 'jbDetAccRate');" style="width:100% !important;text-align: right;">                                                    
                                                                        </td>    
                                                                        <td class="lovtd">
                                                                            <?php
                                                                            if ($canEdt === true) {
                                                                            ...
                                                                            <?php } else { ?>
                                                                                <span><?php echo $trsctnLnCstmrRspnsbl; ?></span>
                                                                            <?php } ?>                                             
                                                                        </td> 
                                                                        <td class="lovtd">
                                                                            <input type="text" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_LineDesc" name="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_LineDesc" value="<?php echo $trsctnLnDesc; ?>" style="width:100% !important;text-align: left;">                                                    
                                                                        </td> 
                                                                        <td class="lovtd">
                                                                            <input type="text" class="form-control" aria-label="..." id="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_DocNum" name="oneSalesRqstdItmLinesRow<?php echo $cntr; ?>_DocNum" value="<?php echo $trsctnLnDocNum; ?>" style="width:100% !important;text-align: left;" readonly="true">                                                    
                                                                        </td>
                                                                        <td class="lovtd">
                                                                            <span style="font-weight:bold;"><?php echo $trsctnLnDocStatus; ?></span>                                                    
                                                                        </td>
                                                                        <td class="lovtd" style="text-align: center;">
                                                                            <button type="button" class="btn btn-default" style="margin: 0px !important;padding:0px 3px 2px 4px !important;" onclick="" data-toggle="tooltip" data-placement="bottom" title="Facility Return">
                                                                                <img src="cmn_images/undo_256.png" style="height:15px; width:auto; position: relative; vertical-align: middle;">Facility Return
                                                                            </button>
                                                                        </td>
                                                                        <td class="lovtd" style="text-align: center;">
                                                                            <button type="button" class="btn btn-default" style="margin: 0px !important;padding:0px 3px 2px 4px !important;" onclick="delHotlChckinDocDetLn('oneSalesRqstdItmLinesRow_<?php echo $cntr; ?>');" data-toggle="tooltip" data-placement="bottom" title="Delete Document Line">
                                                                                <img src="cmn_images/no.png" style="height:15px; width:auto; position: relative; vertical-align: middle;">
                                                                            </button>
                                                                        </td>
                                                                    </tr>
                                                                ...
                                                                $trsctnLnID = -1;
                                                                $trsctnLnStrtDte = $hotlChckinDocStrtDte;
                                                                $trsctnLnEndDte = $hotlChckinDocEndDte;
                                                                $trsctnLnSrvcTyp = "";
                                                                $trsctnLnSrvcTypID = -1;
                                                                $trsctnLnFcltyNum = "";
                                                                $trsctnLnFcltyID = -1;
                                                                $trsctnLnCstmrRspnsbl = $hotlChckinDocSpnsr;
                                                                $trsctnLnCstmrID = $hotlChckinDocSpnsrID;
                                                                $trsctnLnCstmrSiteID = $hotlChckinDocSpnsrSiteID;
                                                                $trsctnLnDesc = "";
                                                                $trsctnLnDocNum = "";
                                                                $trsctnLnDocStatus = "";
                                                                $ln_RentedQty = 1;
                                                                $cntr += 1;
                                                                if ($sbmtdln_SrvsTypID > 0 && $sbmtdln_RoomID > 0) {
                                                                    $trsctnLnSrvcTypID = $sbmtdln_SrvsTypID;
                                                                    $trsctnLnSrvcTyp = getSrvsTypName($trsctnLnSrvcTypID);
                                                                    $trsctnLnFcltyID = $sbmtdln_RoomID;
                                                                    $trsctnLnFcltyNum = getRoomName($trsctnLnFcltyID);
                                                                }
                                                                ?>
                                                                <tr id="oneSalesRqstdItmLinesRow_<?php echo $cntr; ?>" onclick="$('#allOtherInputData99').val($('#oneSalesRqstdItmLinesLinesTable tr').index(this));">                                    
                                                                    <td class="lovtd"><span><?php echo ($cntr); ?></span></td>                                              
