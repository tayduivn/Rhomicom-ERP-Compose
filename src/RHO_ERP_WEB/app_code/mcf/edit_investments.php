...
if (array_key_exists('lgn_num', get_defined_vars())) {
                                ...
                                <div class="col-md-12" style="padding:0px 0px 10px 1px !important;">
                                    ...
                                    <div style="padding:0px 1px 0px 1px !important;float:right;">
                                            ...
                                            if ($trnsStatus == "Approved" || $trnsStatus == "Authorized") {
                                                    $reportTitle = "My Investment Certificate";
                                                    $reportName = "Investment Certificate";
                                                    $rptID = getRptID($reportName);
                                                    $prmID1 = getParamIDUseSQLRep("{:invstmnt_id}", $rptID);
                                                    ...
                                                    <button id="prsFDInvBtn" type="button" class="btn btn-primary" onclick="processFixedDepositInvestment(0);" >Process</button>
                                                        <?php } else if($trnsStatusDsply == "Authorized, Processed") { ?>
                                                    <button id="voidFDInvBtn" type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="processFixedDepositInvestment(1);"><img src="cmn_images/90.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Void&nbsp;</button>
                                                    <?php } ?>                                                    
    ...
    else if ($subPgNo == 5.3) {//LIQUIDATION
        $canAuthorizeLoanWrtoff = true;// test_prmssns($dfltPrvldgs[284], $mdlNm);
        $canProcessLoanWrtoff = true; //test_prmssns($dfltPrvldgs[285], $mdlNm);
        $routingID = -1;
                $pkID = isset($_POST['lqdtnID']) ? cleanInputData($_POST['lqdtnID']) : -1;
        
        $lqdtnCode = "";
        $trnsDte = getStartOfDayDMYHMS();
        $lqdtnTrnsDte = $trnsDte;	
	$trnsBranchID = get_Person_BranchID($prsnid);
        $trnsBranch = getGnrlRecNm("org.org_sites_locations", "location_id", "site_desc||' ('||location_code_name||')'", "pasn.get_prsn_siteid($prsnid)"); 
        $invstmnt_officer_id = $prsnid;
        $invstmnt_officer= getGnrlRecNm("prs.prsn_names_nos", "person_id", "trim(title || ' ' || sur_name ||"
	...
	$lqdtnReason = "";
        ...
        $result = getInvestmentLqdtnDet($pkID);
        while ($row = loc_db_fetch_array($result)) {
            $lqdtnCode = $row[1];
            $lqdtnTrnsDte = $row[2];
            $prsnBranchID = $row[3];
            ...
            $lqdtnCustNm = $row[5];
            $lqdtnReason = $row[6];
            $lqdtnTtlInterest = number_format($row[7], 2);
            $crncy = getGnrlRecNm("mcf.mcf_currencies", "crncy_id", "iso_code", $row[10]);
            //$crncyISOCode = 
            $invstmntNo = $row[11];
            $trnsStatus = $row[8];
...
<div class="col-md-12" style="padding:0px 0px 10px 1px !important;">
            <div style="padding:0px 1px 0px 15px !important;float:left;">
                ...
                <?php if ($vwtypActn != "VIEW") { ?>
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="getInvestmentsLqdtnForm('myFormsModalxLG', 'myFormsModalxLGBody', 'myFormsModalxLGTitle', 'Edit Liquidation', 25, <?php echo $subPgNo; ?>, 0, 'EDIT', <?php echo $pkID; ?>, 'indCustTableRow1');" data-toggle="tooltip" title="Reload Transaction">
                        <img src="cmn_images/refresh.bmp" style="padding-right: 2px; height:17px; width:auto; position: relative; vertical-align: middle;">
                    </button>
                <?php } ?>
                <button type="button" class="btn btn-default" style="height:30px;" onclick="getOneMcfDocsForm_Gnrl(<?php echo $pkID; ?>, 'INVESTMENT LIQUIDATION', 140, 'Investment Liquidation Form');" data-toggle="tooltip" data-placement="bottom" title = "Attached Documents">
                    <img src="cmn_images/adjunto.png" style="height:20px; width:auto; position: relative; vertical-align: middle;">
                </button>                                                    
            </div>
            <div style="padding:0px 1px 0px 1px !important;float:right;">
                <?php if (($trnsStatus == "Incomplete" || $trnsStatus == "Withdrawn" || $trnsStatus == "Rejected") && $vwtypActn != "VIEW") { ?>                                                    
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="saveInvestmentLqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, <?php echo $vwtyp; ?>);"><img src="cmn_images/FloppyDisk.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Save&nbsp;</button>
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="submitWithdrawlqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, 1, 'SUBMIT');"><img src="cmn_images/Emailcon.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Submit&nbsp;</button>   
                    <?php
                } else if ($trnsStatus == "Unauthorized" && $vwtypActn != "VIEW") {
                    ?>  
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="submitWithdrawlqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, 1, 'REJECT');"><img src="cmn_images/withdraw_rqst.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Reject&nbsp;</button>
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="submitWithdrawlqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, 1, 'WITHDRAW');"><img src="cmn_images/withdraw_rqst.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Withdraw&nbsp;</button>                                      
                    <button type="button" class="btn btn-default btn-sm" style="height:30px;" onclick="submitWithdrawlqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, 1, 'AUTHORIZE');"><img src="cmn_images/Stamp-512.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Authorize&nbsp;</button>                                                                                                        
                    ...
                    <button type="button" style="display:none !important;" class="btn btn-default btn-sm" style="height:30px;" onclick="submitWithdrawlqdtn(<?php echo $pgNo; ?>, <?php echo $subPgNo; ?>, 1, 'REVERSE');"><img src="cmn_images/90.png" style="left: 0.5%; padding-right: 5px; height:17px; width:auto; position: relative; vertical-align: middle;">Reverse Approval&nbsp;</button>                               
                <?php } ?>
            </div>
        </div>            
        <form class="form-horizontal" id="riskFactorsForm" style="padding:5px 20px 5px 20px;">             
            <div class="row">      
                <div class="form-group form-group-sm">
                    <input class="form-control" size="16" type="hidden" id="lqdtnId" value="<?php echo $lqdtnId; ?>" readonly=""> 
                    <label for="lqdtnCode" class="control-label col-md-4">Transaction ID:</label>
                    <div  class="col-md-8">
                        <input type="text" readonly="readonly" class="form-control" id="lqdtnCode" placeholder="Transaction ID" value="<?php echo $lqdtnCode; ?>"/>
                    </div>
                </div>
                <div class="form-group form-group-sm">
                    <label for="lqdtnTrnsDte" class="control-label col-md-4">Transaction Date:</label>
                    <div  class="col-md-8">
                        <input type="text" <?php echo $mkReadOnly; ?> class="form-control" id="lqdtnTrnsDte" placeholder="Transaction Date" value="<?php echo $lqdtnTrnsDte; ?>" readonly="readonly"/>
                    </div>
                </div> 
                <div class="form-group form-group-sm">
                    <label for="bnkBranch" class="control-label col-md-4">Branch:</label>
                ...
                <div class="form-group form-group-sm">
                    <label for="lqdtnTrnsDte" class="control-label col-md-4">Investment Officer:</label>
                    <div  class="col-md-8">
                        <input type="text" <?php echo $mkReadOnly; ?> class="form-control" id="lqdtnOfficer" placeholder="Investment Officer" value="<?php echo $invstmnt_officer; ?>" readonly="readonly"/>
                        <input type="hidden" <?php echo $mkReadOnly; ?> class="form-control" id="lqdtnOfficerID" placeholder="Investment Officer" value="<?php echo $invstmnt_officer_id; ?>" readonly="readonly"/>
                    </div>
                </div>
                <div class="form-group form-group-sm">
                    <label for="invstmntNo" class="control-label col-md-4">Investment No:</label>
                    <div  class="col-md-8">
                        <div class="input-group">
                            <input type="text" class="form-control" aria-label="..." id="invstmntNo" value="<?php echo $invstmntNo; ?>" readonly="readonly">
                            <input type="hidden" id="invstmntID" value="<?php echo $invstmntID; ?>">
			    <input type="hidden" id="invstmntID2" value="<?php echo $invstmntID; ?>">
                            <label class="btn btn-primary btn-file input-group-addon" onclick="getLovsPage('myLovModal', 'myLovModalTitle', 'myLovModalBody', 'MCF Customer Investments', 'gnrlOrgID', 'bnkBranchID', '', 'radio', true, '', 'invstmntID', 'invstmntNo', 'clear', 1, '');
                                            ;">
                                <span class="glyphicon glyphicon-th-list"></span>
                            </label>
                            <label class="btn btn-primary btn-file input-group-addon" onclick="dsplyInvstmntForm();" data-toggle="tooltip" data-placement="bottom" title = "View Investment">
                                    <span class="glyphicon glyphicon-info-sign"></span>
                            </label>
                        </div>
                    </div>
                ...
                <div class="form-group form-group-sm">
                    <label for="maturityDte" class="control-label col-md-4">Maturity Date:</label>
                    <div  class="col-md-8">
                        <input type="text" <?php echo $mkReadOnly; ?> class="form-control" id="maturityDte" placeholder="Maturity Date" value="<?php echo $maturityDte; ?>" readonly="readonly"/>
                    </div>
                </div>
                <div class="form-group form-group-sm">
                    <label for="lqdtnCustNm" class="control-label col-md-4">Customer:</label>
                    <div  class="col-md-8">
                        <input type="text" readonly="readonly" class="form-control rqrdFld" id="lqdtnCustNm" placeholder="Customer Name" value="<?php echo $lqdtnCustNm; ?>"/>
                    </div>
                </div>                    
                <div class="form-group form-group-sm">
                    <label for="lqdtnReason" class="control-label col-md-4">Liquidation Purpose:</label>
                    <div  class="col-md-8">
						<div class="input-group">
                            <textarea <?php echo $mkReadOnly; ?> class="form-control rqrdFld" id="lqdtnReason" cols="2" placeholder="Liquidation Purpose" rows="4"><?php echo $lqdtnReason; ?></textarea>
                            <label class="btn btn-primary btn-file input-group-addon" onclick="getLovsPage('myLovModal', 'myLovModalTitle', 'myLovModalBody', 'MCF Liquidation Purposes', 'gnrlOrgID', '', '', 'radio', true, '', 'lqdtnReason', 'lqdtnReason', 'clear', 1, '');
                                            ;">
                                <span class="glyphicon glyphicon-th-list"></span>
                            </label>
                        </div>
                ...
                <div class="form-group form-group-sm">
                    <label for="lqdtnTtlInterest" class="control-label col-md-4">Total Interest:</label>
                    <div  class="col-md-8">
                        <div class="input-group">
                            <label class="btn btn-primary btn-file input-group-addon" onclick="">
                                <?php echo $crncy; ?>
                            </label>
                            <input type="text" class="form-control rqrdFld" aria-label="..." id="lqdtnTtlInterest" value="<?php echo $lqdtnTtlInterest; ?>" readonly="readonly">
                        </div>
                    </div>
                </div>
            </div>
            <?php if ($trnsStatusDsply == "Authorized, Unprocessed") { ?>
                <div class="row" style="float:right;padding-right: 7px;">
                    <button id="prcsLqdtnBtn" type="button" class="btn btn-primary" onclick="processlqdtn(0);" >Process Liquidation</button>
                <?php } else if ($trnsStatusDsply == "Authorized, Processed") { ?>
                    <button id="rvsLqdtnBtn" type="button" class="btn btn-primary" onclick="processlqdtn(1);" >Reverse Liquidation</button>
                </div> 
            <?php } ?>                   
        </form>
        <?php
