    ...
    $(document).ready(function () {
        ...
        $(function () {
            ...
            $('[data-toggle="tabajxprfledt"]').click(function (e) {
                ...
                getNextPrflEDTTab('next', targ, dttrgt);
            });
        });
...
function getNextPrflEDTTab(action, targ, dttrgt) {
    if (typeof action === 'undefined' || action === null || rhotrim(action, '; ') === '')
    {
        action = 'next';
    }
    var rhoAllwdTabs = typeof $("#rhoAllwdTabs").val() === 'undefined' ? 'none' : $("#rhoAllwdTabs").val();
    if (typeof targ === 'undefined' || targ === null || targ === 'none' || rhotrim(targ, '; ') === '')
    {
        var rhoPreviousTab = typeof $("#rhoPreviousTab").val() === 'undefined' ? 'none' : $("#rhoPreviousTab").val();
        var rhoNextTab = typeof $("#rhoNextTab").val() === 'undefined' ? 'none' : $("#rhoNextTab").val();
        if (action === 'next') {
            targ = rhoNextTab;
        } else {
            targ = rhoPreviousTab;
    ...
    if (typeof dttrgt === 'undefined' || dttrgt === null || rhotrim(dttrgt, '; ') === '')
    {
        dttrgt = '&vtyp=0';
    }
    var linkArgs = 'grp=50&typ=1' + dttrgt;
    if (targ === 'none') {
        openATab('#allmodules', linkArgs);
    }
    $(targ + 'tab').tab('show');
    $('li.current').addClass('disabled');
    $('li.current').removeClass('current');
    $(targ + 'Li').removeClass('disabled');
    $(targ + 'Li').addClass('current');
    $(targ).removeClass('hideNotice');
    ...
    if (targ.indexOf('prflHomeEDT') >= 0) {
        ...
        $("#rhoPreviousTab").val('none');
        $("#rhoNextTab").val('#prflAddPrsnDataEDT');

        $("#rhoBackBtn").removeClass('hideNotice');
        $("#rhoNextBtn").removeClass('hideNotice');
        $('#rhoPreviousBtn').addClass('hideNotice');
        $('#rhoSubmitBtn').addClass('hideNotice');
    } else {
        if (targ.indexOf('prflAddPrsnDataEDT') >= 0) {
            ...
            $('#prflCVEDT').addClass('hideNotice');
            $('#prflOthrInfoEDT').addClass('hideNotice');
            $("#rhoPreviousTab").val('#prflHomeEDT');
            if (rhoAllwdTabs.indexOf('#prflCVEDT') !== -1) {
                $("#rhoNextTab").val('#prflCVEDT');
                $("#rhoNextBtn").removeClass('hideNotice');
                $('#rhoSubmitBtn').addClass('hideNotice');
            } else if (rhoAllwdTabs.indexOf('#prflOthrInfoEDT') !== -1) {
                $("#rhoNextTab").val('#prflOthrInfoEDT');
                $("#rhoNextBtn").removeClass('hideNotice');
                $('#rhoSubmitBtn').addClass('hideNotice');
            } else {
                $("#rhoNextTab").val('none');
                $("#rhoNextBtn").addClass('hideNotice');
                $('#rhoSubmitBtn').removeClass('hideNotice');
            }
            $("#rhoBackBtn").addClass('hideNotice');
            $('#rhoPreviousBtn').removeClass('hideNotice');
        } else if (targ.indexOf('prflCVEDT') >= 0) {
            $('#prflAddPrsnDataEDT').addClass('hideNotice');
            $('#prflOrgAsgnEDT').addClass('hideNotice');
            $('#prflHomeEDT').addClass('hideNotice');
            $('#prflOthrInfoEDT').addClass('hideNotice');
            $("#rhoPreviousTab").val('#prflAddPrsnDataEDT');
            $("#rhoNextTab").val('#prflOthrInfoEDT');
...
function getNtnlIDForm(elementID, modalBodyID, titleElementID, formElementID, tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, ntnlIDPrsn)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&ntnlIDpKey=" + pKeyID + "&ntnlIDPersonID=" + ntnlIDPrsn + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function saveNtnlIDForm(elementID, tRowElementID, ntnlIDpKey, ntnlIDPersonID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=3" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, pipeSprtdFieldIDs, extDtColNum, tableElmntID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&addtnlPrsPkey=" + pKeyID +
...
function saveAddtnlDataForm(modalBodyID, addtnlPrsPkey, pipeSprtdFieldIDs, extDtColNum, tableElmntID, tRowElementID, addOrEdit)
{
    ...
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        if (addOrEdit === 'ADD')
        {
            var btnHtml = '<td class=\"lovtd\"><button type="button" class="btn btn-default btn-sm" onclick="getAddtnlDataForm(\'myFormsModalx\', \'myFormsModalxBody\', \'myFormsModalxTitle\', \'addtnlPrsnTblrDataForm\',' +
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=4&addtnlPrsPkey=" + addtnlPrsPkey
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&educBkgrdPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function saveEducBkgrdForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=11" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&workBkgrdPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function saveWorkBkgrdForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=12" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&skillsPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function saveSkillsForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=13" +
                ...
                success: function (data) {
                    ...
                    if (daPersonID > 0 && elementID.trim() !== "") {
                        ...
                        if (formTitle.trim() === "View/Edit My Person Basic Profile") {
                            getBscProfile1Form(elementID, modalBodyID, titleElementID, formElementID, formTitle, daPersonID, 0, pgNo, "EDIT", actionText);
                        } else {
                            getBscProfileForm(elementID, modalBodyID, titleElementID, formElementID, formTitle, daPersonID, 0, pgNo, "EDIT", actionText);
                        }
                    }
                    if (shdSbmt > 0) {
                        setTimeout(function () {
                            ...
                            var dialog2 = bootbox.alert({
                                ...
                                callback: function () {
                                    if (shdSbmt > 0)
                                    {
                                        openATab('#allmodules', 'grp=50&typ=1&pg=2&vtyp=0');
                                    }
                                }
                            });
                        }, 800);
...
function getAttchdDocs(actionText, slctr, linkArgs)
{
    ...
    doAjaxWthCallBck(linkArgs, 'attchdDocsList', 'PasteDirect', '', '', '', function () {
        ...
        $('#attchdDocsTable').wrap('<div class="table-responsive">');
        $('#attchdDocsTblForm').submit(function (e) {
            e.preventDefault();
            return false;
        });
...
function getOneLeaveRqstsForm(pKeyID, actionTxt)
{
    ...
    var lnkArgs = 'grp=50&typ=1&pg=4&vtyp=101&sbmtdExctnID=' + pKeyID;
    doAjaxWthCallBck(lnkArgs, 'myFormsModalNrml', actionTxt, 'Leave Plan Execution (ID:' + pKeyID + ')', 'myFormsModalNrmlTitle', 'myFormsModalNrmlBody', function () {
        ...
        $('#myFormsModalNrml').one('hidden.bs.modal', function (e) {
            ...
            getAllLeaveRqsts('clear', '#allmodules', 'grp=50&typ=1&pg=4&vtyp=0');
            ...
            if (result === true)
            {
                var dialog1 = bootbox.alert({
                    ...
                    callback: function () {
                        if (pKeyID > 0)
                        {
                            openATab('#allmodules', 'grp=50&typ=1&pg=4&vtyp=0');
                        }
                    }
                });
                dialog1.init(function () {
...
function getOneAccrlPlnsForm(pKeyID, actionTxt)
{
    ...
    var lnkArgs = 'grp=50&typ=1&pg=4&vtyp=301&sbmtdPlanID=' + pKeyID;
    doAjaxWthCallBck(lnkArgs, 'myFormsModalNrml', actionTxt, 'Leave Accrual Plan (ID:' + pKeyID + ')', 'myFormsModalNrmlTitle', 'myFormsModalNrmlBody', function () {
        ...
        $('#myFormsModalNrml').one('hidden.bs.modal', function (e) {
            $('#myFormsModalNrmlTitle').html('');
            $('#myFormsModalNrmlBody').html('');
            getAllAccrlPlns('clear', '#plansLv', 'grp=50&typ=1&pg=4&vtyp=3');
            $(e.currentTarget).unbind();
        });
    });
}
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pDivGrpPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePDivGrpForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=5" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pSiteLocPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePSiteLocForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=6" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pGradePkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePGradeForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=7" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pSuprvsrPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePSuprvsrForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=8" +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pJobPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePJobForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=9" +
                "&pJobName=" + pJobName +
                "&pJobType=" + pJobType +
        ...
        tRowElementID, formTitle, vtyp, addOrEdit, pKeyID, personID)
{
    getMsgAsync('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&vtyp=" + vtyp + "&pPositionPkeyID=" + pKeyID + "&sbmtdPersonID=" + personID + "&tRowElmntNm=" + tRowElementID);
    });
}
...
function savePPositionForm(elementID, pKeyID, personID, tRowElementID)
{
    getMsgAsyncSilent('grp=1&typ=11&q=Check Session', function () {
        ...
        xmlhttp.open("POST", "index.php", true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.send("grp=50&typ=1&pg=2&q=UPDATE&actyp=10" +
                        ...
                        if (data.message.indexOf("Success") !== -1) {
                            ...
                            BootstrapDialog.show({
                                ...
                                onshown: function (dialogItself) {
                                    ...
                                    /*$('div.modal-footer').each(function (i, el) {
                                     var ftrText = $(el).html();
                                     if (ftrText.indexOf('RELOAD PDF') !== -1) {
                                     $(el).html(ftrText.replace(/bootstrap4-dialog-button btn-block/g, 'bootstrap4-dialog-button'));
                                     }
                                     });*/
                                    var options = {
                                        height: "520px"/*,
                                ...
                                buttons: [{
                                        ...
                                        id: 'popOKBtnSsn',
                                        label: '<i class="fas fa-sync-alt"></i> RELOAD PDF',
                                        cssClass: 'btn-default',
                                        action: function (dialogItself) {
                                            popDialogItself = dialogItself;
                                            var options = {
